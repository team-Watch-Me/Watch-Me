...
idx = 1
for row in md.itertuples():
    print(f"{idx}) {row.content_name}'s tmdb_id: {row.tmdb_id}")
    idx = idx + 1
...
# 원하는 열 순서
column_order = ['tmdb_id', 'name_kr', 'name_eng', 'plot', 'genre', 'age_rating', 'year', 'running_time', 'streaming_provider', 'country', 'staff', 'actor']
